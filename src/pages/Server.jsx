...
import { Plus, ArrowLeft, Package, Users, Settings, BarChart2, Wifi, WifiOff, Copy, RefreshCw, Trash2, ToggleLeft, ToggleRight, Search, X } from 'lucide-react';
import toast from 'react-hot-toast';
import McItem from '../components/McItem.jsx';
import KitEditor from '../components/kit/KitEditor.jsx';
import KIT_TEMPLATES from '../lib/kitTemplates.js';
...
export default function Server() {
  ...
  const filteredKits = kitSearch
    ? kits.filter(k =>
        ...
        Object.values(k.items||{}).some(item => item.type?.toLowerCase().includes(kitSearch.toLowerCase())))
    : kits;
          ...
          <div>
            <div className="flex items-center justify-between mb-4 gap-3">
              <div className="flex items-center gap-3 flex-1">
                <p className="text-sm text-gray-400 whitespace-nowrap">{kits.length} kit{kits.length!==1?'s':''}</p>
                <div className="relative flex-1 max-w-xs">
                  <Search className="w-3.5 h-3.5 absolute left-2.5 top-1/2 -translate-y-1/2 text-gray-500"/>
                  <input
                    className="w-full bg-[#1e1e2e] border border-[#373750] rounded-lg pl-8 pr-3 py-1.5 text-sm text-white placeholder-gray-500 focus:outline-none focus:border-indigo-500"
              ...
              <div className="flex gap-2">
                <button onClick={() => setShowTemplates(true)}
                  className="flex items-center gap-2 px-3 py-2 bg-[#1e1e2e] hover:bg-[#2a2a3e] border border-[#373750] text-gray-300 rounded-lg text-sm transition-all">
                  📋 Templates
                </button>
            ...
            {showTemplates && (
              <TemplatePicker
                onSelect={(template) => { setSelectedKit({ ...template, kit_id: undefined, isTemplate: true }); setShowTemplates(false); }}
            ...
            {kitsLoading ? (
              ...
              <div className="text-center py-16 bg-[#1e1e2e] border border-[#373750] rounded-xl">
                ...
                <p className="text-gray-400 mb-2">{kitSearch ? 'No kits match your search' : 'No kits yet'}</p>
                {!kitSearch && <p className="text-gray-600 text-sm mb-4">Create your first kit and push it to your server</p>}
                {!kitSearch && (
                  <button onClick={() => setSelectedKit('new')}
...
function TemplatePicker({ onSelect, onClose }) {
  return (
    <div className="fixed inset-0 bg-black/80 flex items-center justify-center z-50 p-4" onClick={onClose}>
      <div className="bg-[#1e1e2e] border border-[#373750] rounded-xl w-full max-w-2xl max-h-[80vh] overflow-y-auto" onClick={e=>e.stopPropagation()}>
        <div className="flex items-center justify-between p-4 border-b border-[#373750] sticky top-0 bg-[#1e1e2e]">
          <h3 className="font-bold text-white">📋 Templates de kits</h3>
          <button onClick={onClose} className="text-gray-400 hover:text-white"><X className="w-5 h-5"/></button>
        </div>
        <div className="p-4 grid sm:grid-cols-2 gap-3">
          {KIT_TEMPLATES.map(t => (
            <button key={t.id} onClick={() => onSelect(t)}
              className="text-left p-4 bg-[#1a1a2e] border border-[#2a2a3e] hover:border-indigo-700 rounded-lg transition-all">
              <div className="flex items-center gap-2 mb-1">
                <McItem materialId={t.icon} size={24}/>
                <span className="font-semibold text-white text-sm">{t.name}</span>
              </div>
              <p className="text-xs text-gray-500">{t.description}</p>
              <p className="text-xs text-gray-600 mt-1">{Object.keys(t.items).length} items</p>
            </button>
          ))}
        </div>
      </div>
